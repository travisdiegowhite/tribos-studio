...
function makeChain(table) {
  ...
  const obj = {};
  const local = { table, payload: null, filters: {} };
  for (const m of ['select', 'order', 'limit', 'is', 'or', 'gte', 'lte', 'not', 'insert', 'upsert']) {
    obj[m] = () => obj;
  }
  obj.eq = (col, val) => { local.filters[col] = val; return obj; };
  obj.update = (payload) => { local.payload = payload; updateCalls.push(local); return obj; };
...
describe('api/arc-refill', () => {
  ...
  it('eases the upcoming quality session and writes only changed rows', async () => {
    ...
    // And it MIRRORS onto calendar_entries — the table /train actually reads.
    // Without this the athlete got the "Plan adapted to your readiness" toast
    // and an unchanged calendar.
    const ceUpdates = updateCalls.filter((u) => u.table === 'calendar_entries');
    expect(ceUpdates.length).toBe(pwUpdates.length);
    expect(ceUpdates[0].payload).toHaveProperty('target_load');
    expect(ceUpdates[0].payload).toHaveProperty('target_duration_min');
    // Titles and notes are regenerated by the arc generator every run, so
    // copying them across would quietly undo an athlete's rename.
    expect(ceUpdates[0].payload).not.toHaveProperty('title');
    expect(ceUpdates[0].payload).not.toHaveProperty('notes');
    // Scoped to the athlete, by id, and never over an entry they pinned.
    for (const u of ceUpdates) {
      expect(u.filters.user_id).toBe('u1');
      expect(u.filters.id).toBeTruthy();
      expect(u.filters.pinned).toBe(false);
    }
  });
