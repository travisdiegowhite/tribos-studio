      ...
      if (recentDates.length === 0) {
        ...
        try {
          ...
          if (backfillResponse.status === 202 || backfillResponse.status === 200) {
            ...
            backfillError = 'Garmin reports this date range was already processed and will not re-deliver it. The original FIT file may be unrecoverable — try uploading the .fit file from Garmin Connect manually.';
          } else {
            const errorText = await backfillResponse.text();
...
async function backfillHistorical(req, res, userId, yearsBack = 2) {
  try {
    ...
    const result = await executeBackfillForUser(userId, accessToken, {
      ...
      delayMs: 300000 // 5 minutes between requests - very conservative, ~1 hour for full backfill
    });
