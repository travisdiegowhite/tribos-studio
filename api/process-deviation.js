...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Use user's timezone for accurate "today". Also pull FTP + adaptive tau
    // constants here (used in step 5) so we only read user_profiles once.
    ...
    // FTP + adaptive tau constants were read once in step 3 (profile). Tau values
    // snapshot onto the training_load_daily row per spec §3.4/§3.5 so future
    // reads can reconstruct which window produced TFI/AFI.
    if (profile?.ftp) {
