...
export async function assembleHeroContext(userId, supabase, timezone = 'America/New_York') {
  ...
  const [
    ...
    supabase
      .from('activities')
      .select('id, name, start_date, rss, tss, distance, moving_time, total_elevation_gain, average_watts, effective_power, type')
      .eq('user_id', userId)
  ...
  const lastRide = lastRideRaw ? {
    ...
    durationSeconds: lastRideRaw.moving_time || 0,
    distanceMeters: lastRideRaw.distance || 0,
