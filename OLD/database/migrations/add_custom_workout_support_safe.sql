-- Migration: Add Custom Workout Support (Safe/Idempotent Version)
-- Description: Extends workout_templates table to support user-created workouts with privacy controls and sharing
-- Date: 2025-11-22
-- Version: 1.1 (Complete - Safe Rerun)
-- Note: This version is safe to run multiple times - it will skip already existing policies
...
-- Mark existing templates as system templates (safe to run multiple times)
UPDATE workout_templates
SET is_system_template = true,
    is_public = true
WHERE created_by_user_id IS NULL
  AND (is_system_template IS NULL OR is_system_template = false);
...
-- Drop old policies (safe even if they don't exist)
...
DO $$
BEGIN
  RAISE NOTICE '✅ Custom Workout Migration Complete!';
  RAISE NOTICE 'New columns added: intensity_factor, focus_area, tags';
  RAISE NOTICE 'New tables created: workout_shares';
  RAISE NOTICE 'RLS policies updated for workout_templates and workout_shares';
  RAISE NOTICE 'Helper functions created: get_accessible_workouts, count_user_custom_workouts, count_workouts_shared_by_user, search_workouts_by_tag';
  RAISE NOTICE 'Next step: Migrate 40 library workouts from hardcoded data to database';
END $$;
