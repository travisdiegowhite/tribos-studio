-- ============================================================================
-- Phase 2 - Part 3: Progression Levels (Fixed - no user_id index)
-- ============================================================================
...
-- Add columns to planned_workouts (NO INDEX on user_id since it doesn't exist!)
...
-- Add workout_date column if it doesn't exist (needed for adaptive training)
ALTER TABLE planned_workouts
  ADD COLUMN IF NOT EXISTS workout_date DATE;

-- Create index on target_zone only (not user_id)
CREATE INDEX IF NOT EXISTS idx_planned_workouts_target_zone
  ON planned_workouts(target_zone) WHERE target_zone IS NOT NULL;

SELECT 'Part 3 Complete: Progression tables created' as status;
