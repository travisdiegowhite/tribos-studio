...
/**
 * Athlete Conversation View
 * Individual conversation with a coach (bidirectional messaging)
 */
const AthleteConversationView = () => {
  const { user } = useAuth();
  ...
  const [coachName, setCoachName] = useState(location.state?.coachName || 'Coach');
  ...
  const loadMessages = async (silent = false) => {
    ...
    try {
      ...
      // Mark messages as read (only messages from coach)
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      setNewMessage('');
      await loadMessages(true); // Silent refresh after sending
    } catch (err) {
      ...
      <Stack spacing="lg">
          ...
          <Stack spacing="md" style={{ minHeight: '400px', maxHeight: '600px', overflowY: 'auto' }}>
            {messages.length === 0 ? (
              ...
              messages.map((message, index) => {
                const isAthlete = message.sender_id === user.id;
                ...
                return (
                  <React.Fragment key={message.id}>
                    ...
                    <Group position={isAthlete ? 'right' : 'left'} align="start">
                      {!isAthlete && (
        ...
        <Alert color="blue" variant="light">
          <Text size="sm">
            ...
            and your coach. Messages auto-refresh every 10 seconds.
          </Text>
        </Alert>
      </Stack>
    </Container>
...
export default AthleteConversationView;
