...
import { Card, Text, Group, Badge, Stack, ActionIcon, Tooltip, Button, Checkbox } from '@mantine/core';
import { ChevronLeft, ChevronRight, Plus, CheckCircle, Circle, Download } from 'lucide-react';
import { supabase } from '../supabase';
import { useAuth } from '../contexts/AuthContext';
import { WORKOUT_TYPES } from '../utils/trainingPlans';
import { exportTrainingPlanToCalendar } from '../utils/calendarExport';
    ...
    try {
      if (!activePlan || plannedWorkouts.length === 0) {
        notifications.show({
          ...
          message: 'Add workouts to your training plan first',
          color: 'yellow'
        });
        return;
      }

      // Load ALL workouts for the entire plan (not just current month)
      ...
      // Convert workouts to include actual dates
      const planStartDate = new Date(activePlan.started_at);
      const workoutsWithDates = allWorkouts.map(workout => {
        const daysSinceStart = (workout.week_number - 1) * 7 + workout.day_of_week;
        const workoutDate = new Date(planStartDate);
        workoutDate.setDate(workoutDate.getDate() + daysSinceStart);
      ...
      // Export to calendar
      const result = exportTrainingPlanToCalendar(
        workoutsWithDates,
        activePlan,
        user?.email
      );
      ...
      notifications.show({
        ...
        message: `${result.workoutsExported} workouts exported to ${result.filename}`,
        color: 'green',
        autoClose: 5000
      });
      ...
      <Group justify="space-between" mb="md">
        ...
        <Group gap="xs">
          {activePlan && plannedWorkouts.length > 0 && (
            <Tooltip label="Export to Google Calendar, Apple Calendar, Outlook">
              <Button
                size="xs"
                variant="light"
        ...
        <>
          ...
          <div style={{
            ...
            {days.map((date, index) => {
              ...
              let borderColor = isToday ? '#32CD32' : '#475569';
              let backgroundColor = isToday ? 'rgba(50, 205, 50, 0.15)' : isPast ? '#2d3748' : '#3d4e5e';

              if (workout && isPast) {
                if (workout.completed) {
                  // Completed workouts get green border
                  borderColor = '#51cf66';
                  backgroundColor = 'rgba(81, 207, 102, 0.15)';
                } else if (workout.workout_type !== 'rest') {
                  // Missed workouts (not rest days) get red border
                  borderColor = '#ff6b6b';
                  ...
                  <Stack gap={4}>
                    <Group justify="space-between" gap={2}>
                      ...
                      {workout && workout.workout_type !== 'rest' && (
                        ...
                        >
                          {workout.completed ? <CheckCircle size={14} /> : <Circle size={14} />}
                        </ActionIcon>
                      )}
                    </Group>
                    {/* Planned workout */}
                    ...
                    {dayRides.length > 0 && (
                      <Text size="xs" c="dimmed">
                        {formatDistance(dayRides.reduce((sum, r) => sum + (r.distance_km || 0), 0), 0)}
                      </Text>
                    )}
                  </Stack>
                </Card>
          ...
          <Stack gap="xs" mt="md">
            <Group gap="xs">
              <Text size="xs" c="dimmed" fw={600}>Workout Types:</Text>
            ...
            {activePlan && (
              <Group gap="md">
                ...
                <Group gap={4}>
                  <div style={{ width: 12, height: 12, borderRadius: 2, backgroundColor: 'rgba(50, 205, 50, 0.15)', border: '2px solid #32CD32' }} />
