        ...
        <Tabs value={activeTab} onChange={setActiveTab} variant="pills" radius="md">
          ...
          <Tabs.Panel value="routing" pt="md">
            <Stack gap="sm">
              ...
              <Box>
                <Text size="xs" fw={500} mb={4} style={{ color: 'var(--color-text-secondary)' }}>
...
export function RouteSettingsButton({ onClick, speedProfile, isImperial = true }) {
  ...
  return (
    <Tooltip label="Route Settings & Pace">
      ...
      >
        <Group gap={4}>
          <Text size="sm" fw={600} c="var(--color-text-primary)">{displaySpeed}</Text>
          <Text size="xs" c="var(--color-text-secondary)">{unit}</Text>
        </Group>
      </Button>
    </Tooltip>
  );
