/**
 * CheckInRecommendation — Tap-to-expand recommendation card with accept/dismiss.
 *
 * Fixed from original: uses onClick toggle instead of useHover (works on mobile).
 *
 * States:
 * 1. Collapsed: Shows action label only
 * 2. Expanded (tap): Shows detail, reasoning, accept/dismiss buttons
...
export default function CheckInRecommendationCard({
  ...
  const isExpanded = expanded || state !== 'idle';
  ...
  return (
    ...
    >
      ...
      <Collapse in={isExpanded}>
        <Stack gap="sm" onClick={(e: React.MouseEvent) => e.stopPropagation()}>
